...
class HistoryCard(CardWidget):
    ...
    def _setup_ui(self):
        ...
        # 操作类型
        type_badge = BodyLabel(f"[{self.entry.operation_type}]")
        type_badge.setStyleSheet("color: #0078d4; font-weight: bold;")
        header_layout.addWidget(type_badge)
        
        # 时间
        time_str = self._format_time(self.entry.timestamp)
        ...
        if self.entry.file_path:
            file_label = BodyLabel(f"📄 {self._shorten_path(self.entry.file_path)}")
            file_label.setStyleSheet("color: #0078d4;")
            brief_layout.addWidget(file_label)
        ...
        # 完整文件路径
        if self.entry.file_path:
            full_path_label = BodyLabel(f"文件: {self.entry.file_path}")
            full_path_label.setStyleSheet("color: #666; font-size: 11px;")
            self.content_layout.addWidget(full_path_label)
        
        # 操作详情
        if self.entry.details:
            details_label = BodyLabel(f"详情: {self.entry.details}")
            details_label.setWordWrap(True)
            details_label.setStyleSheet("color: #333;")
            self.content_layout.addWidget(details_label)
        
        # 代码变更
        if self.entry.code_changes:
            changes_card = CardWidget()
            changes_card.setStyleSheet("background-color: #f5f5f5;")
            changes_layout = QVBoxLayout(changes_card)
            changes_layout.setContentsMargins(12, 12, 12, 12)
            
            changes_title = StrongBodyLabel("代码变更:")
            changes_layout.addWidget(changes_title)
            
            changes_text = BodyLabel(self.entry.code_changes)
            changes_text.setWordWrap(True)
            changes_text.setStyleSheet("font-family: Consolas, monospace; font-size: 11px;")
            changes_layout.addWidget(changes_text)
            
            self.content_layout.addWidget(changes_card)
...
class HistoryView(QWidget):
    ...
    def _create_toolbar(self) -> QWidget:
        ...
        self.type_combo.addItems(["全部", "分析", "修改", "查询", "其他"])
    ...
    def _refresh_history(self):
        ...
        for entry in self.entries:
            # 应用筛选条件
            if type_filter != "全部" and entry.operation_type != type_filter:
                continue
            if search_text:
                match = False
                if entry.file_path and search_text in entry.file_path.lower():
                    match = True
                if entry.details and search_text in entry.details.lower():
                    match = True
                if not match:
                    continue
